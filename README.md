...
O CineTracker é uma aplicação estruturada que oferece uma experiência rica para o usuário, permitindo a navegação fácil entre os filmes e a visualização de detalhes importantes. O projeto utiliza uma combinação de dados locais (`db.json`) e dados dinâmicos da API do TMDB, com uma arquitetura de componentes organizada e clara. Cada parte da aplicação desempenha um papel importante na entrega de uma experiência coesa e eficiente.
